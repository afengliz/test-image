...
echo -e "${GREEN}=== Kaniko Privileged 模式构建镜像测试 ===${NC}"
...
NAMESPACE="imgbuild"
POD_NAME="kaniko-privileged"
POD_YAML="kaniko-pod.yaml"
NEW_IMAGE="registry.kube-system.svc.cluster.local:5000/new-image:latest"

# 1. 检查必要文件
echo -e "\n${YELLOW}[1/6] 检查必要文件...${NC}"
if [ ! -f "build-image" ]; then
    echo -e "${YELLOW}build-image 文件不存在，开始构建...${NC}"
...
# 4. 创建 Pod
echo -e "\n${YELLOW}[4/6] 创建 Pod（Privileged 模式）...${NC}"
...
echo -e "${YELLOW}复制构建程序 build-image...${NC}"
kubectl -n "$NAMESPACE" cp build-image "$POD_NAME:/workspace/build-image"
echo -e "${GREEN}✓ build-image 已复制${NC}"

echo -e "${YELLOW}复制 server/main...${NC}"
...
kubectl -n "$NAMESPACE" exec "$POD_NAME" -- chmod +x /workspace/build-image
kubectl -n "$NAMESPACE" exec "$POD_NAME" -- /workspace/build-image

BUILD_SUCCESS=false
if kubectl -n "$NAMESPACE" exec "$POD_NAME" -- /workspace/build-image 2>&1 | grep -q "镜像构建并推送成功"; then
    BUILD_SUCCESS=true
fi

# 7. 验证镜像
echo -e "\n${YELLOW}[7/6] 验证镜像...${NC}"
VERIFY_POD_NAME="verify-privileged-image-$(date +%s)"
...
echo -e "${YELLOW}注意: 使用了 Privileged 模式，安全性较低${NC}"
