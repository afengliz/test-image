...
echo -e "${GREEN}=== Buildah Go SDK 程序内构建方式测试 ===${NC}"
...
NAMESPACE="imgbuild"
POD_NAME="buildah-demo"
POD_YAML="buildah-pod.yaml"
NEW_IMAGE="registry.kube-system.svc.cluster.local:5000/new-buildah-image:latest"

# 1. 检查必要文件
echo -e "\n${YELLOW}[1/6] 检查必要文件...${NC}"
if [ ! -f "main" ]; then
    echo -e "${YELLOW}main 文件不存在，开始构建...${NC}"
    make build
fi
...
echo -e "${YELLOW}复制构建程序 main (约 2.6MB)...${NC}"
kubectl -n "$NAMESPACE" cp main "$POD_NAME:/workspace/main"
echo -e "${GREEN}✓ main 已复制${NC}"

echo -e "${YELLOW}复制 server/main (约 7.8MB)...${NC}"
...
# 7. 验证镜像
echo -e "\n${YELLOW}[7/6] 验证镜像...${NC}"
VERIFY_POD_NAME="verify-buildah-image-$(date +%s)"
